...
export async function handleMusicUpload(e) {
  ...
  try {
    const url = await uploadMusic(file, currentUser.username);
...
export function removeMusic() {
  if (currentUser.music) {
    deleteMusic(currentUser.music).catch(e => console.error(e));
  }
...
const RADIUS_MAP = {
  sharp:   { radius: '0px',    radiusLg: '0px' },
  default: { radius: '4px',    radiusLg: '10px' },
  soft:    { radius: '12px',   radiusLg: '18px' },
  round:   { radius: '999px',  radiusLg: '999px' },
};

function getShadowMap(color) {
  color = color || '#c8ff00';
  return {
    none:   '',
    accent: '0 0 20px ' + color + '40',
    white:  '0 0 20px rgba(255,255,255,0.08)',
    strong: '0 0 40px ' + color + '80',
  };
}

function getBorderMap(color) {
  color = color || '#c8ff00';
  return {
    default: '1px solid #222',
    accent:  '1px solid ' + color + '99',
...
export function initStyleTab() {
  const r = currentUser.profileRadius || 'default';
  const s = currentUser.profileShadow || 'none';
  const b = currentUser.profileBorder || 'default';
  document.querySelectorAll('#radiusOptions .style-option').forEach(o =>
    o.classList.toggle('selected', o.dataset.radius === r));
  document.querySelectorAll('#shadowOptions .style-option').forEach(o =>
    o.classList.toggle('selected', o.dataset.shadow === s));
  document.querySelectorAll('#borderOptions .style-option').forEach(o =>
    o.classList.toggle('selected', o.dataset.border === b));
  const colorInput = document.getElementById('styleAccentColor');
  if (colorInput) colorInput.value = currentUser.profileStyleColor || '#c8ff00';
  updateStyleColorRow(b, s);
}

function updateStyleColorRow(border, shadow) {
  const row = document.getElementById('styleColorRow');
  if (!row) return;
  const show = (border === 'accent' || border === 'thick' || shadow === 'accent' || shadow === 'strong');
  row.style.display = show ? 'flex' : 'none';
}

export function selectStyleColor(val) {
  currentUser.profileStyleColor = val;
  applyProfileStyle(currentUser);
  scheduleCurrentUserFlush();
}

export function selectRadius(val) {
  currentUser.profileRadius = val;
  document.querySelectorAll('#radiusOptions .style-option').forEach(o =>
    o.classList.toggle('selected', o.dataset.radius === val));
  applyProfileStyle(currentUser);
  scheduleCurrentUserFlush();
}

export function selectShadow(val) {
  currentUser.profileShadow = val;
  document.querySelectorAll('#shadowOptions .style-option').forEach(o =>
    o.classList.toggle('selected', o.dataset.shadow === val));
  updateStyleColorRow(currentUser.profileBorder || 'default', val);
  applyProfileStyle(currentUser);
  scheduleCurrentUserFlush();
}

export function selectBorder(val) {
  currentUser.profileBorder = val;
  document.querySelectorAll('#borderOptions .style-option').forEach(o =>
    o.classList.toggle('selected', o.dataset.border === val));
  updateStyleColorRow(val, currentUser.profileShadow || 'none');
  applyProfileStyle(currentUser);
  scheduleCurrentUserFlush();
}

export function applyProfileStyle(user) {
  const profileEl = document.getElementById('screen-profile');
  if (!profileEl) return;
  const r = RADIUS_MAP[user.profileRadius || 'default'];
  const styleColor = user.profileStyleColor || '#c8ff00';
  const shadow = getShadowMap(styleColor)[user.profileShadow || 'none'];
  const border = getBorderMap(styleColor)[user.profileBorder || 'default'];
  const borderStyle = user.profileBorder || 'default';
  profileEl.style.setProperty('--radius', r.radius);
  profileEl.style.setProperty('--radius-lg', r.radiusLg);
  const card = profileEl.querySelector('.profile-container');
  if (card) {
    card.style.boxShadow = shadow;
    if (border !== '1px solid #222') card.style.border = border;
    else card.style.border = '';
    // Затемнение фона под карточкой если выбрана рамка
    if (borderStyle !== 'none' && borderStyle !== 'default') {
      card.style.background = 'rgba(0,0,0,0.55)';
      card.style.backdropFilter = 'blur(6px)';
      card.style.webkitBackdropFilter = 'blur(6px)';
      card.style.padding = '32px';
      card.style.borderRadius = r.radiusLg;
    } else {
