...
// ── API для работы с музыкой ────────────────────────────────────────────────

const MAX_MUSIC_SIZE = 3 * 1024 * 1024; // 3 MB

export async function uploadMusic(file, username) {
  if (file.size > MAX_MUSIC_SIZE) {
    throw new Error('Файл слишком большой (макс. 3 MB)');
  }
  ...
  const path = storageRef(storage, 'music/' + username + '/' + Date.now() + '_' + file.name);
  await uploadBytes(path, file);
  return await getDownloadURL(path);
}

export async function deleteMusic(url) {
  try {
...
export async function saveUser(user) {
  ...
  await setDoc(doc(db, "users", user.username), clean);
  usersCache[user.username] = clean;
}
