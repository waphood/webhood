...
import { applyProfileStyle } from "./dashboard.js";
...
export function postComment(replyTo = null) {
  ...
  if (currentUser && (asSelf || replyTo !== null)) {
    ...
    comment.authorUid    = currentUser.firebaseUid;
  }
...
export function renderComments(comments) {
  ...
  list.innerHTML = sorted.map(c => {
    ...
    const realAuthor = allUsers[c.nick];
    const isVerifiedAuthor = realAuthor && c.authorUid && realAuthor.firebaseUid === c.authorUid;
    const authorVerified = isVerifiedAuthor && realAuthor.verified;
    ...
    const authorTag = isVerifiedAuthor
      ? `<span class="comment-author" style="${nickStyle}${fontStyle}cursor:pointer;text-decoration:underline;text-underline-offset:3px;" onclick="openUserProfile('${escHtml(c.nick)}')">@${escHtml(c.nick)}</span>`
      : `<span class="comment-author" style="${nickStyle}${fontStyle}">@${escHtml(c.nick)}</span>`;
      ...
      <div class="comment-replies">
        ${c.replies.map(r => {
          ...
          const rPageTag   = r.isOwner
            ? `<span style="font-size:9px;color:var(--text-dim);font-family:'JetBrains Mono',monospace;margin-left:4px;border:1px solid var(--border);padding:1px 5px;border-radius:3px">page</span>` : "";
          const rRealAuthor = allUsers[r.nick];
          const rIsVerifiedAuthor = rRealAuthor && r.authorUid && rRealAuthor.firebaseUid === r.authorUid;
          const rVerified  = rIsVerifiedAuthor && rRealAuthor.verified
            ? `<span class="verified-check" style="width:11px;height:11px;font-size:7px;margin-left:2px">✓</span>` : "";
          const rAuthorTag = rIsVerifiedAuthor
            ? `<span class="comment-author" style="${rNickStyle}${rFontStyle}cursor:pointer;text-decoration:underline;text-underline-offset:3px;" onclick="openUserProfile('${escHtml(r.nick)}')">@${escHtml(r.nick)}</span>`
            : `<span class="comment-author" style="${rNickStyle}${rFontStyle}">@${escHtml(r.nick)}</span>`;
          return `<div class="reply-item">
            <div class="comment-meta">
              ${rAuthorTag}${rVerified}${rPageTag}
    ...
    <div class="comment-item" id="cmt-${i}" style="${c.pinned ? "border-left:2px solid var(--accent);" : ""}">
      <div class="comment-meta">
        ${authorTag}${verifiedTag}${pageTag}${pinnedTag}
